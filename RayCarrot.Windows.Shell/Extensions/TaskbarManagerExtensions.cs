...
namespace RayCarrot.Windows.Shell
{
    ...
    public static class TaskbarManagerExtensions
    {
        ...
        /// This window belong to a calling process associated with the button's application
        /// and must be already loaded.</param>
        public static void SetProgressValue(this TaskbarManager taskbarManager, Progress progress, Form form)
        {
            taskbarManager.SetProgressValue(Convert.ToInt32(progress.Current - progress.Min), Convert.ToInt32(progress.Max - progress.Min), form.Handle);
        }
    }
}